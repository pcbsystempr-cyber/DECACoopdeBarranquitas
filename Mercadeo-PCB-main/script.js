...
// Helper seguro para añadir al carrito por ID (evita problemas con nombres especiales y UUIDs)
function addToCartById(id) {
  const candy = candies.find(c => String(c.id) === String(id));
  if (!candy) return;
  ...
  const currentQty = cart[candy.id] ? cart[candy.id].quantity : 0;
...
function renderCandies() {
  ...
  grid.innerHTML = filtered.map((candy, idx) => {
    ...
    <div class="candy-card glass-card rounded-xl p-5 border border-white/10 hover:border-[#3b82f6] group transition-all duration-300"
      ...
      ` : `
        <button onclick="addToCartById('${candy.id}')"
...
function initNutritionModal() {
  ...
  // Null checks: los elementos pueden estar comentados en el HTML
...
// =====================================================
// DECA COOP — Popup emergente para clientes
// =====================================================
